...
export default {
  ...
  data () {
    return {
      ...
      form: {
        ...
        schoolLevel: JSON.parse(this.$route.query.item).isHighGrade
      }
    }
  },
  created () {
  ...
  methods: {
    ...
    onSubmit () {
      this.$axios.schoolUpdate(this.form).then(res => {
        if (res.data.code === '0') {
...
<style>
.system-update-school {
  width: 800px;
}
.system-update-school .el-select {
  width: 30%;
}
</style>
